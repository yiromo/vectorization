...
def calculate_optimal_batch_size():
    """
    Динамически рассчитывает оптимальный размер пакета 
    на основе доступных системных ресурсов
    """
    ...
    memory_factor = total_memory / (16 * 1024 * 1024 * 1024)  # Базовая память 16 ГБ
    cpu_factor = cpu_count / 8  # Базовое количество ядер 8
    
    # Динамическая настройка размеров пакетов
    document_batch_size = int(BASE_DOCUMENT_BATCH_SIZE * memory_factor * cpu_factor)
    chunk_batch_size = int(BASE_CHUNK_BATCH_SIZE * memory_factor * cpu_factor)
    embedding_batch_size = int(BASE_EMBEDDING_BATCH_SIZE * memory_factor * cpu_factor)
    
    # Ограничения для предотвращения чрезмерного потребления ресурсов
    ...
    return document_batch_size, chunk_batch_size, embedding_batch_size

DOCUMENT_BATCH_SIZE, CHUNK_BATCH_SIZE, EMBEDDING_BATCH_SIZE = calculate_optimal_batch_size()
